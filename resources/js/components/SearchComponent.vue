    ...
    <div class="container">
        ...
        <div class="search-title">
          ...
          <div class="custom-line"></div>
             ...
             <select class="form-control" name="template" v-model="specializationSelected">
        ...
        <select class="form-control" name="starTemplate" v-model="starSelected">
          <option value="" disabled selected>Filtra per indice di gradimento</option>
          <option v-for="star in stars" v-bind:value="star[0]" >
            {{ star[1] }}
          </option>
        </select>

        <select class="form-control" name="reviewTemplate" v-model="reviewSelected">
          <option value="" disabled selected>Filtra per numero di recensioni</option>
          <option v-for="review in reviewRange" v-bind:value="review[0]" >
            {{ review[1] }}
          </option>
        </select>
        
        <button v-on:click="mazzinga(specializationSelected,starSelected,reviewSelected)">Filtra</button>
...
export default {
  ...
  data() {
    return {
      ...
      number: 0,
      arrayLengthTmp: 0,
      stars: [[0,"Qualsiasi valutazione"],
                    [1, "Almeno una stella"],
                    [2, "Almeno due stelle"],
                    [3, "Almeno tre stelle"],
                    [4, "Almeno quattro stelle"],
                    [5, "Cinque stelle"]],
      reviewRange: [[0,"Nessuna recensione"],
  ...
  methods: {
    ...
    startFilter: function (id) {
      axios.get("http://127.0.0.1:8000/api/doctors/filter/" + id)
        .then((response) => {
    ...
    mazzinga: function (specializationId,stars, reviews) {
      axios.get("http://127.0.0.1:8000/api/doctors/filter/" + specializationId)
        .then((response) => {
          this.doctors = response.data.data;
          this.doctorsBackup = response.data.data;
    ...
    starFilter: function (star) {
      ...
      this.doctorsBackup.forEach((doctor) => {
        this.somma = 0;
        this.mediaStars = 0;
        doctor.reviews.forEach((review)=> { this.somma+=review.vote
